...
export const Route = createFileRoute("/rendez-vous")({
  ...
  head: () => ({
    meta: [
      { title: "Prendre rendez-vous en ligne — Pharmacie Du Marché Carouge" },
      {
        name: "description",
        content:
      ...
      { property: "og:title", content: "Prendre rendez-vous — Pharmacie Du Marché Carouge" },
      {
        property: "og:description",
        content: "Réservation en ligne des vaccins et prestations santé à Carouge.",
      },
    ],
  }),
        ...
        <main className="mx-auto max-w-2xl px-5 py-20">
          <div className="rounded-2xl border border-border bg-secondary/40 p-8 text-center">
            ...
            <p className="mt-3 text-sm text-muted-foreground">
              Merci {firstName}, nous vous confirmons votre créneau par téléphone dans les meilleurs
              délais.
            </p>
            <dl className="mt-8 space-y-3 rounded-xl border border-border bg-background p-5 text-left text-sm">
              ...
              <Row
                label="Date"
                value={new Date(`${confirmed.date}T12:00:00`).toLocaleDateString("fr-CH", {
                  weekday: "long",
                  day: "numeric",
                  month: "long",
                  year: "numeric",
              ...
              <Row label="Patient" value={`${firstName} ${lastName}`} />
              <Row label="Téléphone" value={phone} />
              {email && <Row label="E-mail" value={email} />}
            </dl>
            <p className="mt-6 text-xs text-muted-foreground">
              Une question ? Appelez-nous au{" "}
              <a className="font-semibold text-primary" href={PHARMACY.phoneHref}>
                {PHARMACY.phone}
              </a>
            </p>
          </div>
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-5xl px-5 py-14">
        <h1 className="font-serif text-4xl font-semibold text-foreground">Prendre rendez-vous</h1>
        <p className="mt-4 max-w-2xl text-sm leading-relaxed text-muted-foreground">
          Prenez rendez-vous en ligne avec la {PHARMACY.name} Carouge pour vos prestations de santé
          en pharmacie et vos vaccinations.
        </p>
        ...
        <form onSubmit={handleSubmit} className="mt-10 grid gap-10 lg:grid-cols-[1fr_320px]">
          <div className="space-y-8">
            <fieldset>
              ...
              <div className="mt-3 flex gap-3">
                {(
                  [
                    { k: "vaccin", label: "Vaccination" },
                    { k: "prestation", label: "Prestation santé" },
                  ] as const
                ).map((o) => (
                  <button
                    key={o.k}
            ...
            <fieldset>
              <legend className="text-xs font-semibold uppercase tracking-[0.16em] text-foreground">
                2. {kind === "vaccin" ? "Choix du vaccin" : "Choix de la prestation"}
              </legend>
          ...
          <aside className="h-fit rounded-xl border border-border bg-secondary/40 p-6 lg:sticky lg:top-24">
            <h2 className="font-serif text-lg font-semibold text-foreground">Récapitulatif</h2>
            <dl className="mt-4 space-y-3 text-sm">
              <Row label="Type" value={kind === "vaccin" ? "Vaccination" : "Prestation santé"} />
              <Row label="Prestation" value={service || "—"} />
            ...
            <p className="mt-4 flex items-start gap-2 text-xs text-muted-foreground">
              ...
              l’équipe avant le rendez-vous.
            </p>
            <p className="mt-2 flex items-start gap-2 text-xs text-muted-foreground">
              <Phone className="mt-0.5 size-3.5 shrink-0" />
              <a href={PHARMACY.phoneHref} className="hover:text-primary">
                {PHARMACY.phone}
              </a>
            </p>
          </aside>
